...
// sac - add a soecified timeout setting per dial - 17 works to 4 or 5 rings for me
$dial_timeout = "17";
// end - sac 
	...
	switch($state[DIAL_ACTION])
	{
		case DIAL_STATE_DIAL:
			...
			if ($state[DIAL_NUMBER_INDEX] < count($numbers))
			{
				...
				// sac -- add dial timeout
				$dial = $response->addDial(array('action' => current_url(), 'timeout'=>$dial_timeout));
				// end - sac 				 
				if ($dial_whom_selector === 'user-or-group')
				{
					$name = null;
					...
					// sac - creates simultaneous dialing
					while($state[DIAL_NUMBER_INDEX] < count($numbers))
					{
				...
				else
				{
					...
					$dial->addNumber($numbers[$state[DIAL_NUMBER_INDEX]]);
					// sac - moved from below so that User or Group block won't
					// get incremented again incorrectly
						$state[DIAL_NUMBER_INDEX] = $state[DIAL_NUMBER_INDEX] + 1;
					// end - sac 
				}
